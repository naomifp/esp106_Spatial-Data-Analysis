...
```{r}
# msa_trip$MSACAT <- replace(msa_trip$MSACAT, msa_trip$MSACAT == "MSA of 1 million or more, with rail", msa_trip$MSACAT == "MSA ≥ 1 million, with rail")

# msa_trip <- msa_trip$MSACAT[msa_trip$MSACAT == 'MSA of 1 million or more, and not in 1'] <- 'MSA ≥ 1 million, no rail'
# msa_trip <- msa_trip$MSACAT[msa_trip$MSACAT == 'MSA less than 1 million'] <- 'MSA < 1 million'
# msa_trip <- msa_trip$MSACAT[msa_trip$MSACAT == 'Not in MSA '] <- 'Not MSA'
```
...
ggplot(msa_trip, aes(x=MSACAT, y=N, fill=TRIP)) +  
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
        ...
        scale_x_discrete(label = function(MSACAT) lapply(strwrap(MSACAT, width = 20, simplify = FALSE), paste, collapse="\n")) +
